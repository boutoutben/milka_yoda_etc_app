...
const generateAnimalNumberSchema = require('./genierateAnimalNumberSchema');
...
const AdopterProfileSchema = Yup.object().shape({
  ...
  .of(Yup.string().oneOf(['Maison avec jardin',"Maison avec cour", "Maison sans jardin", "Appart avec balcon sécurisé", "Apport sans balcon"], "Le lieu n'est pas dans la liste.")) 
    .min(1, "Il faut choisir au moins une option")
    .max(5, "Il ne faut pas choisir plus de 5 options"),
  ...
  animalCase: Yup.array()
    .of(Yup.string().oneOf([
        "Pas encore",
          ...
          "Rongeur",
          "Autre"
        ], "Ce choix de l'animal n'est pas dans la liste."))
...
module.exports = AdopterProfileSchema;