...
const AddUpdateAnimalSchema = (isRequired=false) => {
  ...
  .test(
    ...
    function (file) {
      ...
      // Récupère le nom de fichier depuis `originalname` ou `filename`
  ...
  .test(
    ...
    const schema = Yup.object().shape({
        ...
        animal: Yup.string()
        .min(3, "L'animal doit comporter au moins 3 caractères.")
        .max(20, "L'animal doit comporter au maximum 20 caractères.")
        .matches(/^[A-Za-zÀ-ÿ\s\-']+$/, "Format invalide")
    ...
    .of(
      Yup.string()
        .required("La chaine de caractère ne peut pas être vide.")
        .matches(/^\d+$/, "Chaque race doit être une chaîne numérique.")
    )
...
module.exports= AddUpdateAnimalSchema;